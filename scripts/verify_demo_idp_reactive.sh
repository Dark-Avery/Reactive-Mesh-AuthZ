...
OUTDIR="${1:-$REPO_ROOT/experiments/results/verify-demo-idp-reactive}"
...
kubectl apply -k "$REPO_ROOT/deploy/kustomize/overlays/reactive-demo-idp" >/dev/null
...
PF_IDP_PID=$!
kubectl port-forward --address 127.0.0.1 -n reactive-mesh-authz svc/receiver "${RECEIVER_PORT}:8080" >/tmp/reactive-mesh-demo-idp-receiver-pf.log 2>&1 &
PF_RECEIVER_PID=$!
kubectl port-forward --address 127.0.0.1 -n reactive-mesh-authz svc/envoy-reactive "${ENVOY_PORT}:8081" >/tmp/reactive-mesh-demo-idp-envoy-pf.log 2>&1 &
PF_ENVOY_PID=$!
sleep 5
...
log_path = pathlib.Path("/tmp/reactive-mesh-demo-idp-reactive.log")
with log_path.open("w", encoding="utf-8") as log:
    ...
    req = urllib.request.Request(
        ...
        data=json.dumps({"event_type": "session-revoked", "sub": sub, "sid": sid, "jti": jti, "reason": "demo_idp_smoke"}).encode(),
...
if not (summary["receiver_status"] == 200 and summary["terminated_after_revoke"] and summary["reopen_denied"]):
    raise SystemExit("verify_demo_idp_reactive failed")
PY
